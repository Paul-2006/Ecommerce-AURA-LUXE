...
import { Truck, Search, Filter, RefreshCw, Eye, ArrowLeft } from "lucide-react";
import { getAdminDeliveryVerificationList } from "../../services/verificationService";
import VerificationBadge from "../../components/verification/VerificationBadge";
import "../../css/AdminPortal.css";

export function DeliveryVerificationList() {
  ...
  const fetchList = async () => {
    setLoading(true);
    try {
      const data = await getAdminDeliveryVerificationList(statusFilter, search);
      setList(data);
    } catch (err) {
      console.error("Error loading delivery partner verifications:", err);
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="admin-portal-page">
      ...
      <div className="page-header flex-between" style={{ marginBottom: "16px" }}>
        <div>
          <span className="badge-pill badge-primary">Logistics Compliance</span>
          <h1 style={{ margin: "4px 0 0 0", fontSize: "1.4rem", fontWeight: 800, color: "#0F172A" }}>
            Delivery Partner Applications
          </h1>
          <p style={{ margin: 0, fontSize: "0.85rem", color: "#64748B" }}>
            Review Driving Licences, Vehicle RC, Insurance validity, and expiry alerts.
          </p>
        </div>
      </div>
      ...
      <div className="table-filter-bar glass-panel" style={{ padding: "12px 16px", marginBottom: "16px", display: "flex", gap: "12px", alignItems: "center", flexWrap: "wrap" }}>
        <form onSubmit={handleSearchSubmit} style={{ flex: 1, display: "flex", gap: "8px", minWidth: "260px" }}>
          <div className="input-with-icon" style={{ flex: 1 }}>
            ...
            <input
              type="text"
              placeholder="Search by Partner Name, DL Number, or Vehicle RC..."
              value={search}
      ...
      {loading ? (
        ...
        <div className="card-empty-state text-center" style={{ padding: "40px 20px" }}>
          <Truck size={36} style={{ color: "#94A3B8", marginBottom: "8px" }} aria-hidden="true" />
          <h3>No Delivery Applications Found</h3>
          <p>No delivery partner records match the selected status or search query.</p>
        </div>
      ) : (
            ...
            <tbody>
              {list.map((item) => (
                <tr key={item.verificationId}>
                  <td>
                    <strong style={{ color: "#0F172A", display: "block" }}>{item.partnerName || "Delivery Agent"}</strong>
                    <span style={{ fontSize: "0.78rem", color: "#64748B" }}>{item.phoneNumber}</span>
                  </td>
                  <td>
                    <code style={{ background: "#F1F5F9", padding: "2px 6px", borderRadius: "4px", fontSize: "0.82rem", fontWeight: 700, color: "#1E293B" }}>
                      {item.drivingLicenceNumber || "NOT_PROVIDED"}
                    </code>
                  </td>
                  <td>
                    <code style={{ background: "#F1F5F9", padding: "2px 6px", borderRadius: "4px", fontSize: "0.82rem", fontWeight: 700, color: "#1E293B" }}>
                      {item.vehicleRegistrationNumber || "NOT_PROVIDED"}
                    </code>
                  </td>
                  <td>
                    <VerificationBadge expiryStatus={item.expiryStatus} />
                  </td>
                  <td>
                  ...
                  <td className="text-right">
                    <button type="button" className="btn btn-primary btn-sm" onClick={() => navigate(`/admin/verification/delivery/${item.verificationId}`)}>
...
export default DeliveryVerificationList;
